...
`air-bridge`는 air-gap 환경에서 데이터를 전송하기 위해 사용하는 도구다.

`air-bridge` is a tool for moving data across air-gapped environments.

이 프로젝트는 `sender`와 `receiver` 두 애플리케이션이 한 쌍으로 동작한다. 주요 전송 흐름은 `encode`, `slide`, `capture`, `decode`로 이어지며, 송신 측에서 파일을 이미지 시퀀스로 변환하고 수신 측에서 이를 다시 복원한다.
...
보조 명령인 `identify`, `pack`, `unpack`는 `sender`를 대상 환경에 반입하기 전에 파일 구성 확인과 패키징 작업을 돕기 위한 기능이다.
...
- [`docs/warning.ko.md`](docs/warning.ko.md)
- [`docs/warning.en.md`](docs/warning.en.md)

## 사용 흐름 Overview / Workflow Overview

일반적인 사용 흐름은 아래와 같다.

1. 대상 컴퓨터에서 전송할 파일에 대해 `encode`를 수행한다.
2. 생성된 이미지 파일을 송출하기 위해 `slide`를 실행한다.
3. 데이터를 수신할 환경에서 `capture`를 수행한다.
4. 수집된 결과를 복원하기 위해 `decode`를 수행한다.
...
`slide`와 `capture`는 USB capture board를 사용하는 구성을 기본 전제로 한다.  
capture board가 없다면 이미지를 수동으로 촬영하거나 별도 방식으로 옮겨 처리할 수 있다.
...
상세한 실행 방법과 배포 방식은 아래 문서를 참고한다.

See the deployment documents below for detailed usage and packaging guidance.

## 빌드 / Build

아래 명령으로 전체 프로젝트를 빌드한다.
...
기본 산출물은 `build/libs/sender-<version>.jar` 와 `build/libs/receiver-<version>.jar` 이다.

The primary artifacts are `build/libs/sender-<version>.jar` and `build/libs/receiver-<version>.jar`.

실행 기준은 `sender` 와 `receiver` 모두 jar다. `receiver` 는 대량 `decode` 나 `capture` 작업에서 JVM 메모리 옵션을 추가하는 것을 권장한다.
...
## 배포 문서 / Deployment Docs

- [warning ko](docs/warning.ko.md)
- [warning en](docs/warning.en.md)
  <!-- - [`sender` deployment](docs/deploy-sender.md) -->
  <!-- - [`receiver` deployment](docs/deploy-receiver.md) -->
