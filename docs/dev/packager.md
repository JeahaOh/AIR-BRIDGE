...
1. 입력 파일을 제자리 rewrite 한다. 이때 쓰기 오류로 인한 원본 훼손을 예방하기 위해,
   덮어쓰기 직전 원본 파일을 충돌 없는 `airbridge-unpack-backup-*` 임시 백업으로 옮겼다가
   성공 시 폐기하고 실패 시 복원하는 롤백 메커니즘을 적용한다. 기존 사용자 파일
   `<입력>.bak`은 백업 용도로 덮어쓰지 않는다.
...
- unpack 중 기존 사용자 `.bak` 파일 보존과 성공 시 임시 백업 미누출
