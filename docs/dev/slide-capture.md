...
단방향 카메라 채널이라 **back-channel이 없다**. 즉 송신측(`slide`)은 수신측(`capture`)이
따라오는지 실시간으로 알 수 없고, **송신측이 스스로 속도를 자동 조정하는 것은 불가능**하다.
실제로 쓰는 페이싱 메커니즘은 세 가지다.
...
1. **수신측 측정 → 권장 속도 출력(사람이 적용).** `capture`는 이번 실행에서 잡은 고유 심볼
   ...
   `= 고유 1개당 평균 간격 × 안전계수 1.3`)를 간결하게 출력한다. 송신측을 자동 제어하는
   채널은 없으므로 어디까지나 **가이드값**이다.
...
- `recommendedDwellMs`가 고유 처리율에 따라 권장 dwell을 산출/판단 보류하는지
