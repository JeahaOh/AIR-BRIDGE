...
(예: `고유 12.3 QR/s -> slide page-display-ms >= 90ms 권장 — 지금 slide Page(ms)가 90보다
작으면 올리고, 크면 90까지 내려도 됩니다`). 수신 측이 송신 측에 직접 신호를
...
`Capture`는 QR을 받으면서 파일별로 심볼이 충분히 모였는지 실시간으로 판정합니다.

- 상태 표시줄에 `복원 가능 N/M 파일`이 함께 표시됩니다(N=복원 가능, M=지금까지 관측된 파일 수).
- 어떤 파일이 복원 가능해지면 로그에 `[CAPTURE][DONE] <파일> — ... decode로 복원 가능` 줄이 뜨고,
  관측된 파일이 전부 복원 가능해지면 `관측된 파일 N개 모두 복원 가능 — slide를 정지해도 됩니다`
  배너가 뜨며 상태 표시줄에도 `✔ 관측된 파일 모두 복원 가능 — slide 정지 가능`이 붙습니다.
- 이때 송신 PC의 `Slide`를 직접 멈추고 `Decode` 탭으로 넘어가면 됩니다(자동 정지는 없습니다).
- 주의: 어떤 파일의 QR이 **한 번도 카메라에 잡히지 않았다면** 그 파일은 집계에 들어가지 않습니다.
  보낸 파일 개수와 `M`(관측 수)이 맞는지 확인한 뒤 멈추세요. 최종 확정은 `Decode` 결과입니다.
