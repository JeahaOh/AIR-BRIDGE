...
DB 조회 결과를 전송해야 한다면 `sender query`로 CSV 결과 폴더를 먼저 만든 뒤,
그 폴더를 `encode --in` 입력 중 하나로 사용합니다. `query`는 QR 전송 파이프라인 내부
단계가 아니라 입력 파일 일부를 만드는 선택 기능입니다.
...
chunk size, QR size, label height, files per folder, encode workers,
repair overhead, targets, skip dirs, exclude, 변환 옵션,
folder structure입니다. 실행 중단이 필요하면
...
복원 성공한 QR PNG는 입력 폴더에서 삭제됩니다. 복원이 이미 끝난 파일의 남은
QR(복구용 여분)도 함께 삭제됩니다.

예:

- `batch/0001.png` 성공 후 해당 PNG 삭제

구버전 실행이 남긴 `*-success` 디렉터리는 계속 건너뛰므로, 오래된 작업 폴더에서도
이미 복원된 QR을 다시 처리하지 않습니다.
