...
- 이미지 확장자 일부와 Java 직렬화 파일(`.ser`)은 목록에 있어도 패킹 대상에서
  제외될 수 있다.
- 이름을 바꿀 수 없는 특이 케이스(이동할 수 없는 이름 충돌, 원래부터
  `target.txt`인 파일, 이름만 `.jar`/`.zip`이고 실제 아카이브가 아닌 파일 등)는
  `WARN ...` 한 줄로 알려 주고 안전한 쪽으로 처리한다. 충돌 대상도 함께 `.txt`로
  이동할 수 있으면 단계적으로 이름을 바꾼다. 내용이 조용히 사라지지 않는다.
